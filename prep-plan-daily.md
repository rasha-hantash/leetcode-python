...
This file is the **curriculum reference** — what each Day's New problems are. Your live Recall + New queue lives in [`prep-data/today.md`](./prep-data/today.md), generated by `uv run prep recompute` (or the LaunchAgent at 8:30 AM). See [`README.md`](./README.md) for the daily flow.

> Skipped a day? Run `uv run prep recompute` — yesterday's untouched problems surface in today's New automatically.
...
- [ ] `uv sync` from the repo root; `uv run pytest` should pass; `uv run prep recompute` generates the first `prep-data/today.md`
