...
def render_coverage(
    ...
    """Render the comprehensive overview file: Readiness → Mocks → System Design → DSA-by-pattern.
    ...
    lines = [
        "# Coverage",
        "",
        "_Generated by `prep recompute`. Comprehensive overview — readiness, mocks, system design, and the full DSA curriculum by pattern. Boxes auto-check from the ledger and per-category JSON files._",
        "",
    ]
    if readiness is not None:
        lines.extend(render_readiness_block(readiness))
