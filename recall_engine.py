...
def render_mocks_md(mocks: list[Mock], today: date) -> str:
    """Standalone view of mock interview state — separate from the daily
    queue so the user always has a high-level view of upcoming + pending
    bookings without opening today.md."""
    ...
    lines = [
        "# Mocks",
        "",
        "_Generated by `prep recompute`. Edit `prep-data/mocks.json` to update state._",
        "",
        f"{_progress_bar(completed, total)} {completed}/{total} complete · "
...
def render_coverage(
    ...
    lines = [
        "# Coverage — by pattern",
        "",
        "_Generated by `prep recompute`. Same set of problems the engine draws from, organized for at-a-glance gaps. Boxes auto-check from the ledger._",
        "",
    ]
    if readiness is not None:
        lines.extend(render_readiness_block(readiness))
...
def recompute(
    ...
    mocks_md_path: Path | None = None,
) -> RecomputeResult:
    """One full cycle: log yesterday's completions, then regenerate today.md
    (with the readiness banner at the top), coverage.md, and the standalone
    mocks.md view if `mocks_md_path` is given."""
    ...
    if mocks_md_path is not None:
        mocks_md_path.parent.mkdir(parents=True, exist_ok=True)
        mocks_md_path.write_text(render_mocks_md(mocks, today))
...
@click.option(
    "--mocks-md",
    type=click.Path(dir_okay=False, path_type=Path),
    default=Path("prep-data/mocks.md"),
    show_default=True,
    help="Generated standalone mocks view. Overwritten on every recompute.",
)
def recompute_cmd(
    daily: Path,
    today_md: Path,
    ...
    """Fold yesterday's checks into the ledger, then regenerate today.md, coverage.md, and mocks.md."""
    result = recompute(
        daily,
        today_md,
        ledger,
